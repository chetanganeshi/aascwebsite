...
<script type="text/javascript">
      ...
      {
        setTimeout(function(){
          $('#light_box').load('manage-departments-update-modal.php?id='+id); /*Modal Content Load from page location*/
    ...
    <form method="post" action="manage-departments.php">
      <table class="table table-bordered">
        ...
        <?php
                                     ...
                                     $result = mysqli_query($conn,"SELECT MAX(id)+1 FROM departments");
        ...
        <tr>
          <td><span style="color: #3973ac; font-weight: bold; ">Add Department</td>
          <td><input type="text" name="txtdept" id="txtdept" class="form-control" style="width: 200px"></td>
        </tr>
...
function delete_row(id)
{
      ...
      $.confirm({
        ...
        buttons: {
            confirm: function () {
               ...
               $.ajax({
               type: 'POST',
               url: 'manage-departments-delete-code.php',
               data: ({ id: id }),
  ...
  else
  {
    $.ajax({
         type: 'POST',
         url: 'manage-departments2.php',
         data: ({id:id, departments:departments}),
         success: function(response3) 
         
     {
             if(response3 == "1")
            {  
              $('.modal-body').html(" Department Added Successfully");
                 $("#myModal").modal();   
                  setTimeout(function(){
                  window.location.href='manage-departments.php';
                  }, 1000); 
             ...
             {
                 $('.modal-body').html("Department Not Added");
                 $("#myModal").modal();
             }
         }
      });
